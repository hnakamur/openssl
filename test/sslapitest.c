...
static int execute_test_ktls_sendfile(int tls_version, const char *cipher)
{
    ...
    if (!TEST_true(create_ssl_objects2(sctx, cctx, &serverssl,
                                       &clientssl, sfd, cfd)))
        goto end;
    OSSL_TRACE3(KTLS, "after create_ssl_objects2, serverssl=%p, clientssl=%p, clientssl->options=0x%lx\n",  serverssl, clientssl, clientssl->options);

    if (!TEST_true(SSL_set_options(serverssl, SSL_OP_ENABLE_KTLS))) {
        OSSL_TRACE(KTLS, "SSL_set_options(serverssl, SSL_OP_ENABLE_KTLS) failed.\n");
        goto end;
    }
    OSSL_TRACE3(KTLS, "SSL_set_options(serverssl, SSL_OP_ENABLE_KTLS) OK, serverssl->options=0x%lx, SSL_OP_ENABLE_KTLS=0x%lx, serverssl=%p.\n", serverssl->options, SSL_OP_ENABLE_KTLS, serverssl);
