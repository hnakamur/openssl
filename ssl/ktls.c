                                ...
                                const EVP_CIPHER_CTX *dd)
{
    ...
    case TLS1_3_VERSION:
        break;
    default:
        OSSL_TRACE2(KTLS, "Skip ktls for s=%p because tls version is %d, nor TLS 1.2 nor 1.3\n", s, s->version);
        return 0;
    }
...
# ifdef OPENSSL_KTLS_AES_CCM_128
    case NID_aes_128_ccm:
        if (s->version == TLS_1_3_VERSION /* broken on 5.x kernels */
          ...
          OSSL_TRACE2(KTLS, "Skip ktls for s=%p NID_aes_128_ccm, TLS 1.3 or tag_length mismatch, (s->version==TLS_1_3_VERSION)=%d.\n", s, s->version == TLS_1_3_VERSION);
          return 0;
        }
# endif
...
# endif
        return 1;
    default:
        OSSL_TRACE2(KTLS, "Skip ktls for s=%p unsupporeted EVP_CIPHER_get_nid(c)=%d\n", s, EVP_CIPHER_get_nid(c));
        return 0;
    }
}
                          ...
                          size_t mac_secret_size)
{
    ...
    if (s->version == TLS1_2_VERSION &&
        ...
        if (!EVP_CIPHER_CTX_get_updated_iv(dd, geniv,
            ...
            OSSL_TRACE1(KTLS, "Skip ktls for s=%p in ktls_configure_crypto case#1\n", s);
            return 0;
        }
        iiv = geniv;
    }
...
# endif
    default:
        OSSL_TRACE1(KTLS, "Skip ktls for s=%p in ktls_configure_crypto case#2\n", s);
        return 0;
    }
