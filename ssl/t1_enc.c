...
#ifndef OPENSSL_NO_KTLS
    if (s->compress) {
        OSSL_TRACE2(KTLS, "Skip ktls because compressed, s=%p, which=%d\n", s, which);
        goto skip_ktls;
    }

    if (((which & SSL3_CC_READ) && (s->mode & SSL_MODE_NO_KTLS_RX))
        || ((which & SSL3_CC_WRITE) && (s->mode & SSL_MODE_NO_KTLS_TX))) {
        OSSL_TRACE2(KTLS, "Skip ktls because of ssl_mode, s=%p, which=%d\n", s, which);
        goto skip_ktls;
    }

    /* ktls supports only the maximum fragment size */
    if (ssl_get_max_send_fragment(s) != SSL3_RT_MAX_PLAIN_LENGTH) {
        OSSL_TRACE2(KTLS, "Skip ktls because of fragment size, s=%p, which=%d\n", s, which);
        goto skip_ktls;
    }
...
# ifdef __FreeBSD__
    ...
    case SSL_AES256:
        if (s->ext.use_etm) {
            OSSL_TRACE2(KTLS, "Skip ktls because of ext.use_etm, s=%p, which=%d\n", s, which);
            goto skip_ktls;
        }
        ...
        case SSL_SHA384:
            ...
            break;
        default:
            OSSL_TRACE2(KTLS, "Skip ktls because of algorithm_mac, s=%p, which=%d\n", s, which);
            goto skip_ktls;
        }
        ...
        break;
    default:
        OSSL_TRACE2(KTLS, "Skip ktls because of algorithm_enc, s=%p, which=%d\n", s, which);
        goto skip_ktls;
    }
...
# endif
    ...
    if (which & SSL3_CC_WRITE) {
       if (BIO_flush(bio) <= 0) {
           OSSL_TRACE2(KTLS, "Skip ktls because flush failed, s=%p, which=%d\n", s, which);
           goto skip_ktls;
       }
    }
...
# ifndef __FreeBSD__
    ...
    if (which & SSL3_CC_READ) {
        ...
        if (count_unprocessed < 0) {
            OSSL_TRACE2(KTLS, "Skip ktls because of count unprocessed records failed, s=%p, which=%d\n", s, which);
            goto skip_ktls;
        }
