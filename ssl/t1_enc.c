...
# endif
    ...
    if (s->compress || (s->options & SSL_OP_ENABLE_KTLS) == 0) {
        fprintf(stderr, "DEBUG_KTLS: Skip ktls for s=%p because compressed or ktls disabled, s->compress=%p, s->options=0x%lx, disabled=%d.\n", s, s->compress, s->options, (s->options & SSL_OP_ENABLE_KTLS) == 0);
        goto skip_ktls;
    }

    /* ktls supports only the maximum fragment size */
    if (ssl_get_max_send_fragment(s) != SSL3_RT_MAX_PLAIN_LENGTH) {
        fprintf(stderr, "DEBUG_KTLS: Skip ktls for s=%p because fragment size is not max, fragment=%u, max=%u\n", s, ssl_get_max_send_fragment(s), SSL3_RT_MAX_PLAIN_LENGTH);
        goto skip_ktls;
    }

    /* check that cipher is supported */
    if (!ktls_check_supported_cipher(s, c, dd)) {
        fprintf(stderr, "DEBUG_KTLS: Skip ktls for s=%p because cipher is not supported.\n", s);
        goto skip_ktls;
    }
    ...
    if (which & SSL3_CC_WRITE) {
       if (BIO_flush(bio) <= 0) {
           fprintf(stderr, "DEBUG_KTLS: Skip ktls for s=%p because BIO_flush failed.\n", s);
           goto skip_ktls;
       }
    }
    ...
    if (!ktls_configure_crypto(s, c, dd, rl_sequence, &crypto_info, &rec_seq,
        ...
        fprintf(stderr, "DEBUG_KTLS: Skip ktls for s=%p because ktls_configure_crypto failed.\n", s);
        goto skip_ktls;
    }
...
# ifndef OPENSSL_NO_KTLS_RX
        ...
        if (count_unprocessed < 0) {
            fprintf(stderr, "DEBUG_KTLS: Skip ktls for s=%p because count_unprocessed=%d < 0\n", s, count_unprocessed);
            goto skip_ktls;
        }
...
# else
        fprintf(stderr, "DEBUG_KTLS: Skip ktls for s=%p because OPENSSL_NO_KTLS_RX is defined.\n", s);
        goto skip_ktls;
# endif
    }

    /* ktls works with user provided buffers directly */
    fprintf(stderr, "DEBUG_KTLS: calling BIO_set_ktls s=%p, which=0x%x, SSL3_CC_WRITE=0x%x, which & SSL3_CC_WRITE=0x%x\n", s, which, SSL3_CC_WRITE, which & SSL3_CC_WRITE);
    if (BIO_set_ktls(bio, &crypto_info, which & SSL3_CC_WRITE)) {
        fprintf(stderr, "DEBUG_KTLS: s=%p BIO_set_ktls(bio, &crypto_info, which & SSL3_CC_WRITE) returned true, which=0x%x, SSL3_CC_WRITE=0x%x, which & SSL3_CC_WRITE=0x%x\n", s, which, SSL3_CC_WRITE, which & SSL3_CC_WRITE);
        if (which & SSL3_CC_WRITE)
 ...
 skip_ktls:
    ...
    fprintf(stderr, "DEBUG_KTLS: s=%p after s->statem.enc_write_state = ENC_WRITE_STATE_VALID.\n", s);
